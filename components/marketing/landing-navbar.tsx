...
const NAV_LINKS = [
  ...
  { label: "100 Product Challenge", id: "challenge", href: "/#challenge" },
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
        {/* Logo */}
        <Link href="/" className="flex items-center shrink-0 text-xl font-extrabold text-slate-900" aria-label="Content Flywheel home">
        ...
        <div className="hidden md:flex items-center gap-4">
          <SignedOut>
            <Link href="/sign-in" className="text-slate-900 hover:text-orange-600 transition-colors text-sm font-medium">
              Sign In
            </Link>
          ...
          <SignedIn>
            ...
            <SignOutButton redirectUrl="/">
              <button className="text-slate-900 hover:text-orange-600 transition-colors text-sm font-medium">
                Sign Out
              </button>
            </SignOutButton>
          </SignedIn>
        ...
        {/* Mobile hamburger */}
        <button
          className="flex items-center justify-center md:hidden w-9 h-9 rounded-lg text-slate-700 hover:text-slate-900 hover:bg-slate-100 transition-colors"
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-slate-200 px-4 py-4 flex flex-col gap-1">
          ...
          <div className="mt-2 pt-2 border-t border-slate-200 flex flex-col gap-2">
            <SignedOut>
              <Link href="/sign-in" onClick={() => setMobileOpen(false)} className="rounded-xl px-4 py-3 text-slate-900 hover:text-orange-600 hover:bg-slate-50 transition-colors text-sm font-medium">
                Sign In
              </Link>
            ...
            <SignedIn>
              ...
              <SignOutButton redirectUrl="/">
                <button className="rounded-xl px-4 py-3 text-slate-900 hover:text-orange-600 hover:bg-slate-50 text-left w-full text-sm transition-colors">
                  Sign Out
                </button>
              </SignOutButton>
            </SignedIn>
