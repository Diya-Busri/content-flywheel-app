  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#0a0a0a]">
      ...
      <div className="mx-auto max-w-7xl px-6 pb-16 pt-20 sm:pt-32 lg:px-8 lg:pt-40">
        ...
        >
          {[
            { label: "1,200+ creators", sub: "in early access" },
            { label: "14,800+ products", sub: "created with AI" },
            { label: "£280k+ revenue", sub: "generated by creators" },
          ].map((s) => (
            <div key={s.label} className="flex items-center gap-2 text-center sm:text-left">
              <div className="h-1 w-1 rounded-full bg-orange-500/60 hidden sm:block" />
              <div>
                <span className="text-sm font-bold text-white">{s.label} </span>
                <span className="text-sm text-white/30">{s.sub}</span>
              </div>
            </div>
          ))}
        </motion.div>
