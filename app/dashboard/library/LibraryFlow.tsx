...
export default function LibraryFlow() {
  ...
  /** Live section progress for generating products: { [productId]: { total, done } } */
  const [generatingProgress, setGeneratingProgress] = useState<Record<string, { total: number; done: number }>>({});
  /** Tracks which product IDs are already being polled so we don't start duplicates. */
  ...
  useEffect(() => {
    const generatingItems = items.filter(i => i.type === "product" && i.status === "generating");
    if (generatingItems.length === 0) return;

    generatingItems.forEach((item) => {
      if (pollingRef.current[item.id]) return; // already polling
      pollingRef.current[item.id] = true;

      const pollOne = async () => {
        while (pollingRef.current[item.id]) {
          ...
          try {
            ...
            const done = sections.filter(s => ((s?.content ?? s?.contentHtml ?? "").trim().length > 0)).length;

            if (total > 0) {
              setGeneratingProgress(prev => ({ ...prev, [item.id]: { total, done } }));
            }

            if (status === "draft" && total > 0 && done === total) {
              pollingRef.current[item.id] = false;
              setItems(prev => prev.map(p => p.id === item.id ? { ...p, status: "draft" } : p));
              setGeneratingProgress(prev => { const n = { ...prev }; delete n[item.id]; return n; });
              return;
            }
            if (status === "failed") {
              pollingRef.current[item.id] = false;
  ...
  }, [items.filter(i => i.status === "generating").map(i => i.id).join(",")]);
