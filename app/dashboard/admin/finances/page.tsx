...
const UK_PERSONAL_ALLOWANCE = 1257000;
const UK_BASIC_RATE = 0.20;
const UK_HIGHER_THRESHOLD = 5027000;
...
export default function AdminFinancesPage() {
  ...
  const [revenueFromOrders, setRevenueFromOrders] = useState(0);
  ...
  useEffect(() => {
    async function load() {
      const [expRes, noteRes, revRes] = await Promise.allSettled([
        fetch("/api/admin/finances/expenses").then((r) => r.json()),
        fetch("/api/admin/finances/notes").then((r) => r.json()),
        fetch("/api/admin/finances/revenue").then((r) => r.json()),
      ]);
      if (expRes.status === "fulfilled" && Array.isArray(expRes.value)) setExpenses(expRes.value);
      if (noteRes.status === "fulfilled" && Array.isArray(noteRes.value)) setNotes(noteRes.value);
      if (revRes.status === "fulfilled" && revRes.value?.total) setRevenueFromOrders(revRes.value.total);
      setLoading(false);
    }
    load();
  }, []);
  ...
  const expensesByCategory = useMemo(() => {
    ...
    for (const e of expenses) map[e.category] = (map[e.category] ?? 0) + e.amountPence;
  ...
  return (
    <div style={{ padding: "32px 24px", maxWidth: "1000px", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      {/* Summary Cards */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "16px", margin: "28px 0" }}>
        {[
          ...
          { label: "Net P&L", value: fmt(profitPence), color: profitPence >= 0 ? "#16a34a" : "#ef4444", bg: profitPence >= 0 ? "#f0fdf4" : "#fef2f2", sub: profitPence >= 0 ? "In profit 🎉" : "Still in the red" },
          { label: "Est. Tax Owed", value: fmt(taxOwedPence), color: "#f59e0b", bg: "#fffbeb", sub: "UK self-assessment" },
          { label: "After Tax", value: fmt(netAfterTaxPence), color: "#6366f1", bg: "#eef2ff", sub: "What you keep" },
          { label: "Owed To You", value: payYourselfBackPence > 0 ? fmt(payYourselfBackPence) : "Paid back ✅", color: payYourselfBackPence > 0 ? "#f97316" : "#16a34a", bg: payYourselfBackPence > 0 ? "#fff7ed" : "#f0fdf4", sub: payYourselfBackPence > 0 ? "Still owed to you" : "Fully recovered!" },
          ...
          <div key={label} style={{ background: bg, borderRadius: "16px", padding: "20px", border: `1px solid ${color}22` }}>
            ...
            <p style={{ margin: "0 0 4px", fontSize: "20px", fontWeight: 800, color, lineHeight: 1 }}>{value}</p>
      ...
      <div style={{ background: "#fffbeb", border: "1px solid #fcd34d", borderRadius: "12px", padding: "12px 16px", marginBottom: "24px", fontSize: "13px", color: "#92400e" }}>
        ⚠️ Tax estimate uses UK 2024/25 rates: £12,570 personal allowance · 20% basic rate up to £50,270 · 40% above. Consult an accountant for your actual return.
      </div>
      ...
      <div style={{ display: "flex", gap: "4px", marginBottom: "24px" }}>
        {(["overview", "expenses", "notes"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            background: tab === t ? "#f97316" : "#f3f4f6",
      ...
      {tab === "overview" && (
        <div>
          ...
          {expensesByCategory.length === 0 ? (
            <p style={{ color: "#9ca3af", fontSize: "14px" }}>No expenses logged yet. Add some in the Expenses tab.</p>
          ...
          {(() => {
            ...
            if (!months.length) return <p style={{ color: "#9ca3af", fontSize: "14px" }}>No data yet.</p>;
            const max = Math.max(...months.map((m) => m[1]));
            return months.map(([month, pence]) => (
      ...
      {tab === "expenses" && (
        <div>
          <div style={{ background: "#fff", borderRadius: "16px", padding: "24px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: "24px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 100px", gap: "12px", marginBottom: "12px" }}>
              <div>
                <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "#374151", marginBottom: "4px" }}>Description</label>
                <input value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} placeholder="Vercel Pro subscription" style={{ width: "100%", padding: "9px 12px", borderRadius: "8px", border: "1px solid #e5e7eb", fontSize: "13px", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "#374151", marginBottom: "4px" }}>Category</label>
                <select value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))} style={{ width: "100%", padding: "9px 12px", borderRadius: "8px", border: "1px solid #e5e7eb", fontSize: "13px", boxSizing: "border-box", background: "#fff" }}>
                  {CATEGORIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
                </select>
              </div>
              <div>
                <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "#374151", marginBottom: "4px" }}>Date</label>
                <input type="date" value={form.date} onChange={(e) => setForm((f) => ({ ...f, date: e.target.value }))} style={{ width: "100%", padding: "9px 12px", borderRadius: "8px", border: "1px solid #e5e7eb", fontSize: "13px", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "#374151", marginBottom: "4px" }}>Amount (£)</label>
                <input type="number" value={form.amount} onChange={(e) => setForm((f) => ({ ...f, amount: e.target.value }))} placeholder="20.00" min="0" step="0.01" style={{ width: "100%", padding: "9px 12px", borderRadius: "8px", border: "1px solid #e5e7eb", fontSize: "13px", boxSizing: "border-box" }} />
              </div>
            </div>
            {formError && <p style={{ color: "#dc2626", fontSize: "13px", marginBottom: "8px" }}>{formError}</p>}
            <button onClick={addExpense} disabled={adding} style={{ padding: "10px 24px", borderRadius: "10px", background: "linear-gradient(135deg,#f97316,#ea580c)", color: "#fff", fontWeight: 700, fontSize: "14px", border: "none", cursor: "pointer" }}>
                    ...
                    return (
                      <tr key={e.id} style={{ borderBottom: "1px solid #f9fafb" }}>
                        ...
                        <td style={{ padding: "13px 16px" }}>
                          <span style={{ display: "inline-block", padding: "2px 10px", borderRadius: "999px", fontSize: "11px", fontWeight: 700, background: color + "15", color, border: `1px solid ${color}33` }}>{catLabel}</span>
                        </td>
                        <td style={{ padding: "13px 16px", fontSize: "15px", fontWeight: 700, color: "#ef4444" }}>{fmt(e.amountPence)}</td>
                        <td style={{ padding: "13px 16px" }}>
                          <button onClick={() => deleteExpense(e.id)} style={{ padding: "4px 10px", borderRadius: "6px", border: "1px solid #fee2e2", background: "#fef2f2", color: "#dc2626", fontSize: "12px", cursor: "pointer" }}>Delete</button>
                        </td>
                      </tr>
                    );
                  })}
      ...
      {tab === "notes" && (
        <div>
          <div style={{ background: "#fff", borderRadius: "16px", padding: "24px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: "24px" }}>
            ...
            <textarea value={noteText} onChange={(e) => setNoteText(e.target.value)} placeholder="e.g. Need to repay £340 card spend from Nov. Set aside 20% of next month's revenue for tax..." rows={4} style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1px solid #e5e7eb", fontSize: "14px", resize: "vertical", boxSizing: "border-box", lineHeight: 1.6 }} />
            <button onClick={addNote} disabled={addingNote || !noteText.trim()} style={{ marginTop: "10px", padding: "10px 24px", borderRadius: "10px", background: "linear-gradient(135deg,#f97316,#ea580c)", color: "#fff", fontWeight: 700, fontSize: "14px", border: "none", cursor: "pointer", opacity: !noteText.trim() ? 0.5 : 1 }}>
              {addingNote ? "Saving…" : "Save Note"}
            </button>
          </div>
          {notes.length === 0 ? (
            <div style={{ textAlign: "center", padding: "48px", color: "#9ca3af", fontSize: "14px" }}>No notes yet.</div>
          ) : notes.map((n) => (
            <div key={n.id} style={{ background: "#fff", borderRadius: "14px", padding: "20px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", border: "1px solid #f3f4f6", marginBottom: "12px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "12px" }}>
                <p style={{ margin: 0, fontSize: "14px", color: "#374151", lineHeight: 1.7, whiteSpace: "pre-wrap", flex: 1 }}>{n.content}</p>
                <button onClick={() => deleteNote(n.id)} style={{ flexShrink: 0, padding: "4px 10px", borderRadius: "6px", border: "1px solid #fee2e2", background: "#fef2f2", color: "#dc2626", fontSize: "12px", cursor: "pointer" }}>Delete</button>
              </div>
