...
export default function VideoGuidePage() {
  ...
  if (error) {
    return (
      <main className="min-h-screen bg-[#0F0F0F] text-white p-6 md:p-10 flex flex-col items-center justify-center">
        <div className="max-w-md text-center">
          <div className="w-16 h-16 rounded-full bg-amber-500/20 flex items-center justify-center mx-auto mb-6">
            <FileText className="w-8 h-8 text-amber-500" />
          </div>
          <h1 className="text-xl font-bold text-white mb-2">No guide found</h1>
  ...
  if (!guide) {
    return (
      <main className="min-h-screen bg-[#0F0F0F] text-white p-6 md:p-10 flex flex-col items-center justify-center">
        ...
        <p className="text-[#A0A0A0]">Loading guide...</p>
      </main>
    );
  }
