/**
 * Generate voiceover via ElevenLabs. Returns audio buffer.
 * Caller must upload to get a public URL for Creatomate.
 */
export async function generateVoice(narration: string): Promise<Buffer> {
  ...
  if (!res.ok) {
    ...
    console.error("[tiktok-shop-demo] ElevenLabs error:", res.status, err);
