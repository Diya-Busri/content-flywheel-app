...
async function pollRenderStatus(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (!statusRes.ok) {
      ...
      console.error("[tiktok-shop-demo] Creatomate poll failed:", statusRes.status, errBody);
    ...
    console.log("[tiktok-shop-demo] Render poll", attempt + 1, "status:", data.status);

    if (data.status === "succeeded") {
      if (data.url) {
        console.log("[tiktok-shop-demo] Render done, url:", data.url);
        return data.url;
      }
    ...
    if (data.status === "failed") {
      ...
      console.error("[tiktok-shop-demo] Creatomate render failed:", msg);
...
/**
 * Send render request to Creatomate, poll status until succeeded or failed, return final video URL.
 */
export async function renderWithCreatomate(payload: CreatomatePayload): Promise<string> {
  ...
  console.log("[tiktok-shop-demo] Creatomate create, template_id:", payload.template_id);
  ...
  if (!createRes.ok) {
    console.error("[tiktok-shop-demo] Creatomate create error:", createRes.status, responseBody);
  ...
  if (!renderId) {
    console.error("[tiktok-shop-demo] No render id:", responseBody);
    throw new Error("Creatomate did not return render id");
  }

  console.log("[tiktok-shop-demo] Render created, id:", renderId);
