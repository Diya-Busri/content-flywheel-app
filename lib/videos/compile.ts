...
async function renderImageSegment(
  ...
  // Scale up for smooth zoom then zoompan to 1920x1080
  const filter =
    `[0:v]scale=${WIDTH}:-2,setsar=1:1,crop=${WIDTH}:${HEIGHT},` +
    `scale=8000:-1,zoompan=z='min(zoom+0.001,1.5)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${dFrames}:s=${WIDTH}x${HEIGHT}:fps=${FPS}[v]`;
  const args = [
    "-y",
    "-loop", "1",
    "-i", imagePath,
...
/** Map UI transition names to FFmpeg xfade transition= value. */
 ...
 * transition: optional UI value (fade | slideLeft | slideRight | wipe | zoom); default fade.
 */
export async function compileVideoToFile(
  ...
  const xfadeTransition = (transition && XFADE_TRANSITION_MAP[transition]) ? XFADE_TRANSITION_MAP[transition] : "fade";
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    if (imageUrl && !videoUrl) {
      ...
      await renderImageSegment(inputPath, dur, segPath);
    } else if (videoUrl) {
      ...
      const segPath = join(workDir, `seg_${i}.mp4`);
      await renderVideoSegment(inputPath, dur, segPath);
    } else {
      throw new Error(`Scene ${i + 1} must have image_url or video_url`);
    }
  }

  // 3) Concat with xfade and mix voiceover
  // Offsets for xfade: transition starts at (cumulative duration - xfade). offset_k = sum(durations[0..k]) - (k+1)*XFADE_DURATION
  let cumulative = 0;
  const offsets: number[] = [];
  for (let k = 0; k < durations.length - 1; k++) {
    cumulative += durations[k];
    offsets.push(cumulative - (k + 1) * XFADE_DURATION);
  }

  const segInputs = scenes.map((_, i) => ["-i", join(workDir, `seg_${i}.mp4`)]).flat();
  // filter_complex: [0:v][1:v]xfade=...:offset=o0[v1]; [v1][2:v]xfade=...:offset=o1[v2]; ... ; [vLast]format=yuv420p[vout]
  let filterStr: string;
  if (scenes.length === 1) {
    filterStr = "[0:v]format=yuv420p[vout]";
  } else {
    const filterParts: string[] = [];
    for (let k = 0; k < scenes.length - 1; k++) {
      const inp = k === 0 ? "[0:v]" : `[v${k}]`;
      const next = `[${k + 1}:v]`;
      const out = `[v${k + 1}]`;
      const offset = offsets[k].toFixed(2);
      filterParts.push(
        `${inp}${next}xfade=transition=${xfadeTransition}:duration=${XFADE_DURATION}:offset=${offset}${out}`
      );
    }
    const lastLabel = `[v${scenes.length - 1}]`;
    filterStr = filterParts.join(";") + ";" + lastLabel + "format=yuv420p[vout]";
  }
